...
- [x] T2.4a Worker: Telegram forum methods + `createTelegramClient` factory — `814bcb0`. Worker 195 → **206**, typecheck clean. **Pure refactor held: 311 added / 0 deleted in the test file**, all 195 pre-existing tests unedited (the T1.3 bar). `createTelegramClient(botToken)` returns bound wrappers over the standalone functions, which stay exported; call sites updated in `notifications.ts` and `webhook.ts`. `ForumTopic` typed with all four documented fields **including `is_name_implicit`** — I doubted that one and was wrong: it is real ("True, if the name of the topic wasn't specified explicitly by its creator and likely needs to be changed by the bot"), verified against core.telegram.org. Noted for T2.7/T2.11: an implicitly-named topic is one Telegram expects the bot to rename.
- [x] T2.4b Worker: topic manager — reservation protocol with TTL, steal, and CAS finalize — `290f013`. Worker 206 → **214**, typecheck clean, suite still 3.85s (injected `delayFn`, no real sleeps). All three review findings implemented: 120s TTL + `stealReservation` CAS; `finalize` and `deleteBySession` both gained `AND message_thread_id IS NULL` and switched to `meta.changes`; 429 deletes the reservation and propagates `rate_limited` rather than falling back. Concurrency test asserts `createCalls === 1` **explicitly**, not merely relying on the mock rejecting a second call, and both callers receive the same thread id.
  - Non-vacuity proven on all three: removing the reservation ⇒ two `createForumTopic` calls; making `finalize` unconditional ⇒ the compensating-delete test fails because the late winner's finalize wrongly succeeds; making 429 fall back ⇒ the propagation test fails.
  - **Residual accepted and documented in code** (`topic-manager.ts`, JSDoc + inline after the create succeeds): a winner that creates the Telegram topic then dies before `finalize` leaves an orphan *Telegram* topic with no D1 row, invisible to the reaper. Unavoidable without an idempotent create, which Telegram does not offer; cleanup is manual.
  - Not wired into `handleSendNotification` — that is T2.5.
- [x] T2.5 Daemon + worker: `title`/`dir`/`threaded` end-to-end **through the outbox** — `d38bce7`. Daemon 647 → **650**, worker 214 → **218**, plugin 279 unchanged; typecheck clean modulo the 4 known `lease-cas` errors. **The trap was avoided**: the dead notifier classes were not touched, and each edited file was confirmed on the live path by grep (outbox upsert at `app.ts:421`/`:541` inside the `/stop` and `/question-asked` handlers; `OutboxSender` constructed at `index.ts:279`; `handleSendNotification` mounted at worker `index.ts:53`). `dir` comes from **`session.cwd`** on the daemon side — never the worker's `sessions.label`, which `/current-state` overwrites with titles.
  - Gate is `if (topicsEnabled(env) && threaded !== false)` in `handleSendNotification`.
  - **Flag-off equivalence is genuinely pinned — I re-proved this myself, because the implementer's check tested the wrong thing.** They broke the `topicsEnabled` predicate, which merely fails T2.2's own unit test. The property that matters is the *gate*, so I removed `topicsEnabled(env)` from the condition: the flag-off test failed as required, **and 19 pre-existing notification tests failed with it**. That breadth is the real evidence that flag-off is byte-equivalent to Phase 1, which is what Checkpoint 2a's dark deploy rests on.
  - Stale line numbers in this task's spec, now corrected: `app.ts` `:414-418` and `:518-551`; `outbox-sender.ts` `:149-160` and `:192-197`; `index.ts` `:284`; `poller.ts` `:8-16`; `notifications.ts` `:11-13`, `:176`, `:223-239`, `:246`.
- [x] T2.6 Worker: reopen-on-closed — `5d85125`. Worker 218 → **221**. Closed row ⇒ `reopenForumTopic` then `markOpen` before sending. Failure handling preserves "never drop a notification": `rate_limited` propagates as 429 (outbox pauses and retries); `thread_not_found` deletes the row and falls through to the creator flow; a **generic** error still returns the existing thread id so the send is attempted anyway — if the topic really is unpostable, `sendMessage` fails and T2.8's General fallback catches it. **Unverified and deliberately not guessed:** whether an admin bot can post into a *closed* topic at all. Marked in `topic-manager.ts` as a Checkpoint 2a live-Telegram question.
- [x] T2.7 Worker: stale-thread recovery — `1f21090`. Worker 221 → **224**. `thread_not_found` ⇒ delete the row, recreate **once**, retry the send. Recursion is structurally impossible: the recovery is a single `if` block, not a loop, so a second failure falls through to normal error handling.
  - **The CAS conflict was resolved correctly rather than by weakening the guard.** T2.4b's `deleteBySession` is guarded on `message_thread_id IS NULL`, which would block deleting a *finalized* row. Instead of relaxing it, a separate unguarded `deleteTopicBySession` was added for out-of-band recovery. Verified by grep: the unguarded form has exactly two callers, both stale-recovery (`topic-manager.ts:75`, `notifications.ts:264`), and the guarded form retains its single reservation-cleanup caller (`topic-manager.ts:114`). The single-winner race stays safe.
  - **The `thread_not_found` classifier was NOT tightened.** T1.3's substring match is still unverified against live Telegram; tests pin current behaviour only. Confirm the real string at Checkpoint 2a.
- [x] T2.8 Worker: non-429 fallback to General, 429 propagates — `8c55e79`. Worker 224 → **228**. **Most of this was already built by T2.4b/T2.6** — the audit found six of seven branches already existed and were tested (create 429/non-429, reopen 429/`thread_not_found`/generic, `sendMessage` 429). Exactly one real gap: a **non-429 `sendMessage` failure after a thread id was resolved** returned 502 and dropped the notification. Now retries without `messageThreadId`, delivering to General. `editForumTopic`/`rename` confirmed unreachable during notification dispatch.
