-- pigeon.lua - Pigeon remote control via neovim RPC
--
...
--   3. Register the instance: :PigeonRegister backend
--   4. From outside nvim:
--      nvim --server /tmp/nvim-claude.sock \
--        --remote-expr "luaeval('require(\"pigeon\").dispatch(_A)', '<base64_payload>')"
...
function M.register(name)
  if not name or name == "" then
    vim.notify("pigeon: name is required", vim.log.levels.ERROR)
    return false
  end
  ...
  if vim.bo[bufnr].buftype ~= "terminal" then
    vim.notify("pigeon: current buffer is not a terminal", vim.log.levels.ERROR)
    return false
  end

  local job_id = vim.b[bufnr].terminal_job_id
  if not job_id then
    vim.notify("pigeon: no terminal job found in current buffer", vim.log.levels.ERROR)
    return false
  end
  ...
  vim.notify(string.format("pigeon: registered instance '%s' (buf=%d, job=%d)", name, bufnr, job_id), vim.log.levels.INFO)
  return true
end
...
function M.unregister(name)
  if instances[name] then
    instances[name] = nil
    vim.notify(string.format("pigeon: unregistered instance '%s'", name), vim.log.levels.INFO)
    return true
  end
  return false
end
...
function M.setup()
  -- :PigeonRegister <name> - Register current terminal as a named Claude instance
  vim.api.nvim_create_user_command("PigeonRegister", function(opts)
  ...
  -- :PigeonUnregister <name> - Unregister an instance
  vim.api.nvim_create_user_command("PigeonUnregister", function(opts)
  ...
  -- :PigeonList - Show all registered instances
  vim.api.nvim_create_user_command("PigeonList", function()
    local names = M.list()
    if #names == 0 then
      vim.notify("pigeon: no instances registered", vim.log.levels.INFO)
    else
      vim.notify("pigeon: registered instances: " .. table.concat(names, ", "), vim.log.levels.INFO)
    end
  end, {
    desc = "List all registered Claude Code instances",
  })

  -- :PigeonSend <name> <command> - Send a command to an instance (for testing)
  vim.api.nvim_create_user_command("PigeonSend", function(opts)
    ...
    if not name or command == "" then
      vim.notify("Usage: :PigeonSend <instance-name> <command>", vim.log.levels.ERROR)
      return
    end

    local result = handle_send(name, command)
    if result.ok then
      vim.notify(string.format("pigeon: sent command to '%s'", name), vim.log.levels.INFO)
    else
      vim.notify(string.format("pigeon: failed to send - %s", result.error), vim.log.levels.ERROR)
    end
  end, {
    nargs = "+",
  ...
  vim.api.nvim_create_autocmd("TermOpen", {
    group = vim.api.nvim_create_augroup("Pigeon", { clear = true }),
  ...
  vim.api.nvim_create_autocmd({ "TermClose", "BufWipeout" }, {
    group = vim.api.nvim_create_augroup("Pigeon", { clear = false }),
    callback = function(ev)
      local bufnr = ev.buf
      for name, data in pairs(instances) do
