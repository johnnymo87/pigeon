...
export async function ingestKillCommand(input: KillCommandInput): Promise<void> {
  const { commandId, sessionId, chatId, opencodeClient, sendTelegramReply, sendAck } = input;
  ...
  try {
    ...
    await sendTelegramReply(chatId, `Session \`${sessionId}\` terminated.`);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    console.error(`[kill-ingest] failed to terminate session sessionId=${sessionId}: ${message}`);
    await sendTelegramReply(chatId, `Failed to kill session: ${message}`);
  }
}
