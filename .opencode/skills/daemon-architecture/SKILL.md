...
- `DELETE /session-origin?session_id=` -- ops hard reset, idempotent. The only way back down out of an override. Afterwards declared writers may re-quiet the session later, but with no title layer, DELETE now makes a session louder, full stop.
